...
function investigarGrampeador(){
    ...
    rl.question("\n Dentro do grampeador, tem um bilhete: 'Sou cheio buracos, mas seguro a água. O que sou?'", (resposta) => {
        if(resposta.trim().toLowerCase() == "esponja"){
            console.log("Acertou! Você desbloqueou o chave que estava grampeador")
...
function investigarPapeis(){
    if(resolvidos.papeis){
        console.log("Voce já resolveu esse desafio")
        return menuPrincipal()
    }
    rl.question("\nUm dos papéis diz: 'Qual palavra está sempre escrita errada nodicionário?'", (resposta) => {
        if(resposta.trim().toLowerCase() == "errada"){
            console.log("Acertou! Você encontrou a chave no meio dos papéis")
            resolvidos.papeis = true
        }else{
            console.log("Errou! Não e isso")
        }
        menuPrincipal()
    })
}
